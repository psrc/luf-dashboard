...
dt_rgs_server <- function(id, paths, runs, tsyear, baseyear) {
  moduleServer(id, function(input, output, session) {
    ...
    rgsdt <- reactive({
      ...
      for (r in 1:length(runnames)) {
        for (i in 1:length(attribute)){
          ...
          datatable.rgs <- datatable[, lapply(.SD, sum), by = "rgs_id", .SDcols = cols][order(rgs_id)]
    ...
    cntyrgsdt <- reactive({
      ...
      for (r in 1:length(runnames)) {
        for (i in 1:length(attribute)){
          ...
          datatable.cnty.rgs <- datatable[, lapply(.SD, sum), by = c("county", "rgs_id"), .SDcols = cols][order(county, rgs_id)]
    ...
    output$table_a <- renderDT({
      ...
      t <- dcast.data.table(tsRGs, name ~ year + indicator + run, value.var = "value")

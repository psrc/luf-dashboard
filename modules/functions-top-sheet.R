...
calc.cols.tsTable <- function(table, tsyear, runs, baseyear){
  ...
  if(baseyear_run1 == tsyear & baseyear_run2 == tsyear){
    # find columns starting with year
    cols_to_copy <- grep(tsyear, names(table), value = TRUE)
    
    # copy and rename
    t1[, paste0(cols_to_copy, "_b") := .SD, .SDcols = cols_to_copy]
    by_run1 <- paste0(base_run1, "_b")
    by_run2 <- paste0(base_run2, "_b")

    setcolorder(t1, c("Sector", by_run1, by_run2, base_run1, base_run2))
    
  } else if((baseyear_run1 == tsyear) & (baseyear_run2 != tsyear)) {
    # baseyear_run1 <- paste0(base_run1, "_b")
    by_run1 <- paste0(base_run1, "_b")
    t1 <- t1[, (by_run1) := get(eval(base_run1))]
    setcolorder(t1, c("Sector", by_run1, base_run2, target_run1, target_run2))
    
  } else if((baseyear_run2 == tsyear) & (baseyear_run1!= tsyear)) {
    # baseyear_run2 <- paste0(base_run2, "_b")
    by_run2 <- paste0(base_run2, "_b")
    t1 <- t1[, (by_run2) := get(eval(base_run2))]
    
    setcolorder(t1, c("Sector", base_run1, by_run2, target_run1, target_run2))
  } else {
...
create.exp.tsTable <- function(table, runs, tsyear, baseyear){
  ...
  if(b1 == tsyear && b2 == tsyear) {
    ...
    # copy and rename
    table[, paste0(cols_to_copy, "_b") := .SD, .SDcols = cols_to_copy]
    b1_run1_pop_new <- paste0(b1, "_", "Population","_", runnames[1], "_b") #b1 run1
    b2_run2_pop_new <- paste0(b2, "_", "Population","_", runnames[2], "_b") #b2 run2
    b1_run1_emp_new <- paste0(b1, "_", "Employment","_", runnames[1], "_b")
    b2_run2_emp_new <- paste0(b2, "_", "Employment","_", runnames[2], "_b")
  ...
  if((b1 == tsyear) & (b2 != tsyear)) {
    ...
    all_columns <- replace(all_columns, c(2,3,8,9), c(b1_run1_pop_new, b2_run2_pop_new, b1_run1_emp_new, b2_run2_emp_new))
  }
